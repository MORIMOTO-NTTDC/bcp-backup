...
set si_url=http://10.144.16.10/bcp-web/webapi

:CHECK_DIR
set DIST_CURRENT_FOLDER=..\BCP_RELEASE_INSTALL\%YYYYMMDD%_%BRANCH_NUM%
...
XCOPY "BCP_Backup_Boot\bin\release" "%DIST_FOLDER%\boot" /E /F /H /K /R /Y /C /V

CALL :replace %DIST_FOLDER%\boot\BCP_Backup_Boot.exe.config %si_url%

mkdir %DIST_FOLDER%\client

XCOPY "BCP_Backup_Client\bin\release" "%DIST_FOLDER%\client" /E /F /H /K /R /Y /C /V

CALL :replace %DIST_FOLDER%\client\BCP_Backup_Client.exe.config %si_url%

mkdir %DIST_FOLDER%\service

XCOPY "BCP_Backup_Service\bin\release" "%DIST_FOLDER%\service" /E /F /H /K /R /Y /C /V

CALL :replace %DIST_FOLDER%\service\BCP_Backup_Service.exe.config %si_url%

mkdir %DIST_FOLDER%\tool
XCOPY "BCP_Backup_UploadCommand\bin\release" "%DIST_FOLDER%\tool" /E /F /H /K /R /Y /C /V
XCOPY "BCP_Backup_GetStatusCommand\bin\release" "%DIST_FOLDER%\tool" /E /F /H /K /R /Y /C /V
